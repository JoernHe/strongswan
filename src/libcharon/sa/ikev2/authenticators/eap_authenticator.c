										  ...
										  bool do_identity)
{
	...
	if (this->method)
	{
		...
		if (this->method->initiate(this->method, &out) == NEED_MORE)
		{
			...
			if (vendor)
			{
				DBG1(DBG_IKE, "initiating EAP vendor type %N-%N method (id 0x%02X)",
					 eap_type_names, type, pen_names, vendor, out->get_identifier(out));
			}
			else
			{
			...
			if (this->method->set_nonce)
			{
				this->method->set_nonce(this->method, this->sent_nonce);
			}
			if (this->method->set_ike_sa_init)
			{
				this->method->set_ike_sa_init(this->method, this->received_init);
			}
			if (this->method->set_reserved)
			{
	...
	if (vendor)
	{
		DBG1(DBG_IKE, "%s EAP vendor type %N-%N method failed",
					  action, eap_type_names, type, pen_names, vendor);
	}
	else
	{
										 ...
										 eap_payload_t *in)
{
	...
	switch (this->method->process(this->method, in, &out))
	{
		...
		case SUCCESS:
			...
			if (vendor)
			{
				DBG1(DBG_IKE, "EAP vendor specific method %N-%N succeeded, "
					 "%sMSK established", eap_type_names, type, pen_names, vendor,
					 this->msk.ptr ? "" : "no ");
			}
			else
			{
...
failure:
			...
			if (vendor)
			{
				DBG1(DBG_IKE, "EAP vendor specific method %N-%N failed for "
					 "peer %Y", eap_type_names, type, pen_names, vendor,
										 ...
										 eap_payload_t *in)
{
	...
	if (this->method == NULL)
	{
		if (vendor)
		{
			DBG1(DBG_IKE, "server requested vendor specific EAP method %N-%N (id 0x%02X)",
						eap_type_names, type, pen_names, vendor, in->get_identifier(in));
		}
		else
		{
		   ...
		   (conf_type != type || conf_vendor != vendor))
		{
			if (conf_vendor)
			{
				DBG1(DBG_IKE, "requesting EAP method %N-%N, sending EAP_NAK",
						eap_type_names, conf_type, pen_names, conf_vendor);
			}
			else
			{
	...
	if (vendor)
	{
		DBG1(DBG_IKE, "vendor specific EAP method %N-%N failed",
			 eap_type_names, type, pen_names, vendor);
	}
	else
	{
